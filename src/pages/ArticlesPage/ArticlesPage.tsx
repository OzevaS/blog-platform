...
const ArticlesPage = () => {
  const { page, limit } = useAppSelector((state) => state.articlesReducer);
  ...
  return (
    <section className="centered-content">
      {isLoading && <Loader />}
      {isError && <Alert message="Error" type="error" description="Не удалось загрузить статьи" />}
      {hasData && !isError && <ArticleList articles={data.articles} />}
      <Pagination
